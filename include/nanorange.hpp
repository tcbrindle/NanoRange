...
struct PairLike_req {
    ...
    template <typename T>
    auto requires_(T t) -> decltype(valid_expr(
        std::enable_if_t<Integral<std::tuple_size<T>::value>, int>{},
...
template <typename T, typename U, typename V>
NANO_CONCEPT PairLikeConvertibleFrom =
...
template <typename I, typename S>
subrange_kind default_subrange_kind<I, S, true>
        = subrange_kind::sized;
