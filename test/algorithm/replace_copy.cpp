...
void test_iter() {
	...
	int ib[sa] = {0};
	stl2::replace_copy_result<InIter, OutIter> r = stl2::replace_copy(InIter(ia), Sent(ia + sa), OutIter(ib), 2, 5);
...
TEST_CASE("alg.replace_copy")
{
	...
	// Test projection
	{
		...
		P out[stl2::size(in)] = {};
		stl2::replace_copy_result<P *, P *> r = stl2::replace_copy(in, out, 2, P{5, "5"}, &std::pair<int, std::string>::first);
