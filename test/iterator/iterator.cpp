...
iterator_dispatch() { return category::output; }
...
struct arbitrary_iterator<C, EC, R> {
	...
	requires EC
	||
	__stl2::DerivedFrom<C, __stl2::forward_iterator_tag>;
...
arbitrary_iterator<C, B, R> operator+(
		arbitrary_iterator<C, B, R>,
		typename arbitrary_iterator<C, B, R>::difference_type);
...
{
	{
		...
		CHECK(!copy(__stl2::begin(a) + 1, __stl2::end(a) - 1,
					__stl2::begin(b) + 1));
		CHECK(b[0].value == 4);
		CHECK(b[1].value == 1);
		CHECK(b[2].value == 2);
...

	{
		...
		CHECK(copy(__stl2::begin(a) + 2, __stl2::end(a) - 2,
				   __stl2::begin(b) + 2));
		CHECK(b[0] == 7);
		CHECK(b[1] == 6);
		CHECK(b[2] == 2);
